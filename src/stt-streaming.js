// Streaming Speech-to-Text via Deepgram Nova or the OpenAI Realtime API.
//
// This module manages a persistent WebSocket connection for real-time
// transcription with interim results and automatic reconnection. Both providers
// are normalized to the same callback shape: onInterim always receives the
// cumulative text of the utterance so far, and onTranscript the finalized text.
...
class OpenAIRealtimeSTT {
  ...
  _handleEvent(event) {
    switch (event.type) {
      ...
      // OpenAI streams incremental deltas where Deepgram sends a cumulative
      // snapshot of the utterance so far. Accumulate here so both providers
      // emit the same shape and callers don't have to care which is in use.
      case 'conversation.item.input_audio_transcription.delta':
        if (event.delta) {
          this._utterance += event.delta;
          this.onInterim(this._utterance.trim());
        }
        break;
...
// Priority: Deepgram (lowest latency) > OpenAI Realtime > none
// ============================================================================
...
function createStreamingSTT(settings, channel, callbacks) {
  ...
  // No streaming-capable key: the caller falls back to manual scrolling.
  return { type: 'none', provider: 'none', instance: null };
}
